...
wait: '[ ⏳] Wait a moment...',
eror: '[ ❗ ] Failed, try again ^_^',
success: '[ ✅ ] Finished',
error: {
stick: '[ ❗ ] Reply/tag the sticker',
Iv: 'Invalid link!'
},
only: {
group: '『 ⛔ 』This feature is group-specific',
owner: '『 ⛔ 』Special Owner Bot',
admin: '『 🚫 』Group admin only',
Badmin: '『 ⚠️ 』Make bot as admin to use this command'
}
}
...
fakeyt(` *「 GROUP LINK DETECTOR 」*\nYour send link group, sorry you now kick in the group`)
setTimeout(() => {
herman.groupRemove(from, [kic]).catch((e) => { fakeyt(`ERROR, BOT AT LAST NOW ADMIN!`) })
}, 0)
}
...
fakeyt(`「 *VIRTEX DETECTOR* 」\n\nYour send virtex, sorry you now kick in the group`)
...
case 'menu':
fakeyt(`TO APPEAR MENU..`)
teks = `Hii I am ${botname} 👋
...
Dj Ghost - Viral Tiktok mp3

03:42 ━━━━━━●───── 05:20
⇆ㅤ    ◁ㅤ     ❚❚     ㅤ▷ ㅤ  ㅤ↻﻿`
img = fs.readFileSync('./image/herman.jpg')
trans = `Dont spam bot !! Spam/Call Auto Banned!`
but = [
...
⌜𝑆𝑇𝐼𝐶𝐾𝐸𝑅 𝑀𝐸𝑁𝑈⌟
𖥂 ${prefix}stiker _reply picture_
𖥂 ${prefix}ttp _text_
𖥂 ${prefix}attp _text_
𖥂 ${prefix}toimg _reply sticker_
𖥂 ${prefix}patrick
...
⌜𝐾𝑂𝐷𝐸 𝑀𝐸𝑁𝑈⌟
 ...
 𖥂 ${prefix}kodebahasa`
img = fs.readFileSync('./image/herman.jpg')
trans = `hello ${pushname} 👋`
but = [
          { buttonId: `${prefix}rent`, buttonText: { displayText: 'Rent Botz' }, type: 1 },
...
case 'rent':
case 'rentbot':
daftar = `Hello ${pushname}
━━━━━ • ஜ • ❈ • ஜ • ━━━━━
• List Price Rent Botz♨️
🔖 : 3H=2k, 5H=3k
🔖PERMINGGU : 1M=5k, 2M=10k, 5M=20k
🔖PERBULAN : 1B=25k, 6B=50k, 8B=65k
🔖PERTAHUN : 1TH=80k, 3THN=95k, 5THN=125k
🔖PERMANENT : NOT READY!
Contact now owner before payment type #owner 
━━━━━ • ஜ • ❈ • ஜ • ━━━━━
`
...
//࿇ ══━━━━━━━━━━━━━━━━━━━━✥SUBSCRIBE HERMAN CHANEL✥━━━━━━━━━━━━━━━━━━━━━━━━══ ࿇
                ...
                fakeyt(mess.wait)
                    ...
                    ini_txt += `Eternal : ${get_result.thn_kelahiran}\n`
                    ini_txt += `Age : ${get_result.age}\n`
                    ini_txt += `Place : ${get_result.place}\n`
                    ini_txt += `Story : \n${get_result.story}`
                    reply(ini_txt)
                    break
                case 'jadwalsholat':
                ...
                fakeyt(mess.wait)
                    ...
                    ini_txt += `Date : ${get_result.tanggal}\n`
...
//࿇ ══━━━━━━━━━━━━━━━━━━━━✥SUBSCRIBE HERMAN CHANEL✥━━━━━━━━━━━━━━━━━━━━━━━━══ ࿇
           case 'ttp':  
           fakeyt(mess.wait)
                    if (!c) return reply(`Please Enter Your Text\nExample :\n${prefix}attp  herman Botz Whatsapp`)
                    anu1 = await getBuffer(`https://api.xteam.xyz/ttp?file&text=${c}`)
...
fakeyt(`\`\`\`Welcome Activated\`\`\` *${groupMetadata.subject}*`)
} else if ((args[0]) === 'off') {
if (!isWelkom) return fakeyt('*welcome sudah off sebelumnya !!*')
_welkom.splice(from, 1)
fs.writeFileSync('./database/welcome.json', JSON.stringify(_welkom))
fakeyt(`\`\`\`Welcome Disabled\`\`\` *${groupMetadata.subject}*`)
} else {
fakeyt('*Select on to activate, off to disable*')
}
break
case 'antilink':
if (!isGroup) return fakeyt(mess.only.group)
...
fakeyt(`\`\`\`Antilink Activated\`\`\` *${groupMetadata.subject}*`)
} else if (args[0].toLowerCase() === 'off'){
let anu = _antilink.indexOf(from)
_antilink.splice(anu, 1)
fs.writeFileSync('./database/antilink.json', JSON.stringify(_antilink))
fakeyt(`\`\`\`Antilink Disabled\`\`\` *${groupMetadata.subject}*`)
} else {
...
if (!q) return fakeyt(`Pilih on atau off`)
if (args[0].toLowerCase() === 'on'){
if (isAntiVirtex) return fakeyt(`Activated`)
_antivirtex.push(from)
fs.writeFileSync('./database/antivirtex.json', JSON.stringify(_antivirtex))
fakeyt(`\`\`\`Successfully Enable antivirtex in group\`\`\` *${groupMetadata.subject}*`)
} else if (args[0].toLowerCase() === 'off'){
let anu = _antivirtex.indexOf(from)
_antivirtex.splice(anu, 1)
fs.writeFileSync('./database/antivirtex.json', JSON.stringify(_antivirtex))
fakeyt(`\`\`\`Successfully Disable antivirtex in group\`\`\` *${groupMetadata.subject}*`)
} else {
fakeyt(`_Select on Or off_`)
}
break
case 'group':
case 'grup':
...
if (!isBotGroupAdmins) return fakeyt(mess.only.Badmin)
if (args[0] === 'open') {
fakeyt(`*Successfully opened group ${groupMetadata.subject}*`)
herman.groupSettingChange(from, GroupSettingChange.messageSend, false)
} else if (args[0] === 'close') {
fakeyt(`*Successfully Close group ${groupMetadata.subject}*`)
...
teks = 'Permission accepted, promote your admin :\n'
...
mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
if (mentioned.length > 1) {
teks = 'Permission accepted, demote your admin :\n'
for (let _ of mentioned) {
teks += `@${_.split('@')[0]}\n`
}
mentions(teks, mentioned, true)
...
if (!isGroupAdmins && !mek.key.fromMe) return fakeyt(mess.only.admin)
if (!isBotGroupAdmins) return fakeyt(mess.only.Badmin)
if (args.length < 1) return fakeyt('❎ Enter Numbers Correctly')
...
herman.sendMessage(from, `\`\`\`Successful Change of Group Name\`\`\` *${body.slice(9)}*`, text, { quoted: ftrol })
break
case 'setdesc':
if (!isGroup) return fakeyt(mess.only.group)
if (!isGroupAdmins) return fakeyt(mess.only.admin)
...
herman.sendMessage(from, `\`\`\`Successfully Changed Group Desk\`\`\` *${groupMetadata.subject}* Menjadi: *${body.slice(9)}*`, text, { quoted: ftrol })
break
case 'setppgrup':
case 'setpp':
if (!isGroup) return fakeyt(mess.only.group)
